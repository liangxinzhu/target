...
schema0("OwnerReference").

#modeh(schema(const(schema1))).
schema1("Time").

#modeh(schema(const(schema2))).
schema2("LoadBalancerStatus").

#modeh(schema(const(schema3))).
schema3("ServiceExternalTrafficPolicy").
...
schema19("ManagedFieldsOperationType").

#modeh(schema(const(schema20))).
schema20("ServiceType").
...
schema23("SessionAffinityConfig").

#modeh(schema(const(schema24))).
schema24("ServicePort").

#modeh(schema(const(schema25))).
schema25("LoadBalancerIngress").
